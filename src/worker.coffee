...
class Drone
  ...
  _gotRange: (value) ->
    return unless value? and @id? and not @_stopped()
  ...
  _requestRange: ->
    return unless @id? and not @_stopped()
  ...
  _sendResult: ->
    return unless @_data.result? and @id? and not @_stopped()
  ...
  start: ->
    return unless @_data? and @_range? and not @_stopped()
    ...
    if @_data.result?
      ## allow time for any incoming stop messages to be processed
      me = @
